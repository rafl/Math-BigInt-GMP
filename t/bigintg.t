...
ok (ref($x),'Math::GMP'); ok (${_str($C,$x)},123); ok (${_str($C,$y)},321);
...
# _dec, _inc
$x = _new($C,\"7"); ok (${_str($C,_inc($C,$x))},8);
$x = _new($C,\"7"); ok (${_str($C,_dec($C,$x))},6);
...
$x = _new($C,\"123456789");
ok (_check($C,$x),0);
ok (_check($C,123),'123 is not a reference to Math::GMP');
